...
Or use the direct path: `<your-tenant>/ui/kubernetes`
...
Copy the `kubectl apply` command from the Dynatrace wizard and run it in the Terminal:

```bash
# Example — use the command generated by the Dynatrace UI for your tenant
kubectl apply -f https://github.com/Dynatrace/dynatrace-operator/releases/latest/download/kubernetes.yaml
kubectl apply -f dynakube.yaml   # the file from the wizard
```
